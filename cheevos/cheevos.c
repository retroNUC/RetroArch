...
/* Write locals into config string representation */
void rcheevos_leaderboards_settings_write(void)
{
   settings_t* settings = config_get_ptr();
   char* str            = settings->arrays.cheevos_leaderboards_enable;
   char buffer[64];
   ...
   strlcpy(str, buffer, sizeof(buffer));
}

/* Read config string representation into locals */
...
void rcheevos_leaderboards_settings_changed(void)
{
   ...
   /* If hardcore enabled, leaderboards are enabled */
   rcheevos_locals.leaderboards_enabled = rcheevos_locals.hardcore_active;



   /* ...unless leaderboard system is completely off?   CHECK THIS */
